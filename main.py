...
from utils.datasets import DatasetManager as DM
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='DOS (Dual-Space) Project - Multi-space episodic training')

    # Required arguments
    parser.add_argument(
        'config_file', help='Path to the configuration YAML file')
    ...
    parser.add_argument('--samples', '-s', type=int,
                        help='Number of samples (overrides config file)')
...
def main():
    ...
    if args.output_dir:
        config['output']['train-history'] = os.path.join(
            args.output_dir, "train_history")
        config['output']['best_models'] = os.path.join(
            args.output_dir, "models/best_model_")
    ...
    for i, scenario in enumerate(config['scenarios']):
        ...
        if config['mode'] == 'prototypical':
            config_setup_name, distances_weights = get_scenario_name(
                scenario, config['mode'])
        else:
            config_setup_name = scenario

        print_section(
            f"Starting scenario {i+1}/{total_scenarios}: {config_setup_name}")
