...
# MLflow setup
mlflow.set_experiment("Multi-Label Classification Experiment")
...
with mlflow.start_run(run_name="Random Forest Feature Selection"):
    ...
    features_RFC = ftr_importance_RFC[ftr_importance_RFC.Weight >=
                                      ftr_importance_RFC.Weight.mean()].Column.values

    mlflow.log_param("num_features", len(features_RFC))
    mlflow.sklearn.log_model(RFC_pipeline, "random_forest_model")
...
for i in range(n_experiments):
    with mlflow.start_run(run_name=f"Experiment_{i+1}"):
        ...
        selected_model = MLP_MultiLabel(n_features=len(
            features_RFC), n_classes=y_train_data.shape[1])
        ...
        mlflow.log_metric("best_balanced_accuracy", best_acc)
        ...
        for epoch, acc in enumerate(experiment_history["balanced_accuracy"]):
            mlflow.log_metric("balanced_accuracy", acc, step=epoch)
