...
def calculate_metrics(y_true, y_pred, y_pred_prob=None):
    ...
    # Balanced accuracy
    metrics['balanced_accuracy'], _ = multi_label_balanced_accuracy(
        y_true, y_pred)
    ...
    metrics['auprc'] = (average_precision_score(
        ...
        label_binarize(y_pred, classes=np.arange(n_classes)),
        average='macro'))
...
def calculate_all_metrics(y_true, y_pred, y_prob=None):
    ...
    if y_prob is not None:
        n_classes = y_true.shape[1] if len(
            y_true.shape) > 1 else len(np.unique(y_true))
        ...
        metrics["auprc"] = average_precision_score(
            y_true_bin, y_prob_bin, average='macro')
